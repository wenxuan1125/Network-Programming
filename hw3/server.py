    ...
    while True:
        ...
        if (cmd_list[0] == 'login'):
            ...
            if (login == False):
                ...
                sql = 'select username, status from chatroom where username = \'' + login_user + '\';'
                result = sql_conn.execute(sql)
                result = result.fetchall()  # result from cursor_object to list
                if (len(result) != 0):
                    if(result[0][1] == 'open'):
                        c_tcp.send(b'Please do "attach" and "leave-chatroom" first.')
                    else:
                        login = False
            ...
            if (login == False):
                ...
                port = cmd_list[1]
                sql = 'select username from chatroom where username = \'' + login_user + '\';'
                result = sql_conn.execute(sql)
                result = result.fetchall()  # result from cursor_object to list
                if (len(result) == 0):
                    #print('*')
                    sql = '''insert into chatroom (username, port, status) 
                    values(\''''+ login_user + '\', \'' + port + '\', \'open\');'
                    sql_conn.execute(sql)
                    sql_conn.commit()
                    c_tcp.send(b'Start to create chatroom...')
                    #print('*')
                else:
                    #print('#')
                    c_tcp.send(b'User has already create the chatroom.')
                    #print('#')
        elif (cmd_list[0] == 'join-chatroom'):
            if (login == False):
                c_tcp.send(b'Please login first.')
            else:
                chatroom_name = cmd_list[1]
                
                sql = 'select username, port, status from chatroom where username = \'' + chatroom_name + '\';'
                result = sql_conn.execute(sql)
                result = result.fetchall()  # result from cursor_object to list
                if (len(result) == 0):
                    c_tcp.send(b'The chatroom does not exist or the chatroom is close.')
                else:
                    #print(result[0][2])
                    if (result[0][2] == 'close'):
                        c_tcp.send(b'The chatroom does not exist or the chatroom is close.')
                    else:
                        response = 'Action: connection to chatroom server. ' + str(result[0][1])
                        c_tcp.send(response.encode())
        elif (cmd_list[0] == 'leave-chatroom'):
            sql = 'update chatroom set status = \'close\' where username = \'' + login_user + '\';'
            sql_conn.execute(sql)
            sql_conn.commit()
            ...
            if (login == False):
                ...
                result = result.fetchall()  # result from cursor_object to list
                if (len(result) == 0):
                    c_tcp.send(b'Please create-chatroom first.')
                elif (result[0][1] == 'open'):
                        c_tcp.send(b'Your chatroom is still running.')
                else:
                    sql = 'update chatroom set status = \'open\' where username = \'' + login_user + '\';'
                    sql_conn.execute(sql)
                    sql_conn.commit()
            ...
            if (login == False):
                ...
                if (len(result) == 0):
                    c_tcp.send(b'Please create-chatroom first.')
                elif (result[0][1] == 'close'):
                        c_tcp.send(b'Please restart-chatroom first.')
                else:
                    c_tcp.send(b'Welecome to the chatroom.')
            ...
            if (login == False):
                ...
                response = 'Chatroom_name Status\n'
                sql = 'select username, status from chatroom;'
                result = sql_conn.execute(sql)
                for row in result:
                    response = response + row[0] + ' ' + row[1] + '\n'
                c_tcp.send(response.encode())
...
sql_conn.execute('''create table if not exists chatroom
       ( username text primary key not null,
       port integer not null,
       status text not null); ''')

try: 
    sql_conn.execute('drop table chatroom;')
    sql_conn.execute('''create table if not exists chatroom
       ( username text primary key not null,
       port integer not null,
       status text not null); ''')
